...
export default class ExportarCamaras extends Resources{
	constructor(props){
		super(props)
	}
	...
	getAll(){
		...
		console.log("ACA");
		SQL('SELECT predios.id, predios.nombre,predios.codigo, camaras.id as camara_id from camaras join colmenas on camaras.colmena_id = colmenas.id join  predios on predios.id = colmenas.predio_id order by 1  desc').then(predios=> {console.log("PREDIOS"); this.send(predios)  })
	}
	...
	get(id){
		...
		// 	let nodo = nodos[0]
			...
			let url ='/files/excels/camaras/predio'+id+'.xlsx'
